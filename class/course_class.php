...
class Course extends Connection{
	...
	public function get_prerequisites($course_id){
		// return associative array or false
		return $this->fetchOne("select * from course_prerequisite inner join courses on course_prerequisite.course_id = courses.course_id where course_id = '$course_id'");
	}

	public function is_prerequisite($course_id){
		return $this->fetchOne("select * from course_prerequisite inner join courses on course_prerequisite.course_prerequisite_id = courses.course_id  where course_prerequisite.course_prerequisite_id='$course_id'");
	}
